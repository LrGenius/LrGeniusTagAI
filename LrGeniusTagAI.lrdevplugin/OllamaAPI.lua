            ...
            if decoded ~= nil then
                if decoded.done_reason == 'stop' then
                    if prefs.replaceSS then
                        decoded.message.content = string.gsub(decoded.message.content, "ß", "ss")
                    end
                    ...
                    if decoded.done_reason ~= nil then
                        ErrorHandler.handleError('Ollama API request failed. Done reason: ' .. decoded.done_reason .. ',  Response: ' .. Util.dumpTable(decoded.response))
                ...
                ErrorHandler.handleError('Ollama API request failed', 'Response from Ollama could not be decoded: ' .. response)
                return false, 'Response from Ollama could not be decoded‚', 0, 0
            end
        else
            ErrorHandler.handleError('Ollama API request failed', 'Got empty response from Ollama for file: ' .. LrPathUtils.leafName(filePath))
            return false, 'Got empty response from Ollama', 0, 0
        end 
    else
        ErrorHandler.handleError('OllamaAPI POST request failed', 'HTTP headers: ' .. Util.dumpTable(headers) .. ' HTTP Response: ' .. (response or 'nil'))
